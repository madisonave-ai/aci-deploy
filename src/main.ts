...
async function main() {
  try {
    ...
    let containerGroupInstance: ContainerInstanceManagementModels.ContainerGroup =
      {
        ...
        imageRegistryCredentials: taskParams.registryUsername
          ? [
              {
                server: taskParams.registryLoginServer,
                username: taskParams.registryUsername,
                password: taskParams.registryPassword,
              },
            ]
          : [],
        ipAddress: {
    ...
    let containerDeploymentResult = await client.containerGroups.createOrUpdate(
      taskParams.resourceGroup,
      taskParams.containerName,
...
): ContainerInstanceManagementModels.ResourceRequirements {
...
function getPorts(
  taskParams: TaskParameters
): Array<ContainerInstanceManagementModels.Port> {
