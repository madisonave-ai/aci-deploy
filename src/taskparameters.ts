...
export class TaskParameters {
  ...
  private constructor(endpoint: IAuthorizer) {
    ...
    let secureEnvironmentVariables = core.getInput(
    ...
    if (gpuSku && !gpuCount) {
      throw Error(
        "You need to specify the count of GPU Resources with the SKU!"
      );
    } else {
      if (gpuCount && !gpuSku) {
        throw Error(
          "GPU SKU is not specified for the count. Please provide the `gpu-sku` parameter"
        );
      }
      this._gpuCount = parseInt(gpuCount);
      this._gpuSKU =
        gpuSku == "K80" ? "K80" : gpuSku == "P100" ? "P100" : "V100";
    }
    ...
    if (!["Linux", "Windows"].includes(osType)) {
      throw Error(
        `The Value of OS Type must be either Linux or Windows only - got ${osType}!`
      );
    } else {
    ...
    if (!["Always", "OnFailure", "Never"].includes(restartPolicy)) {
      throw Error(
        'The Value of Restart Policy can be "Always", "OnFailure" or "Never" only!'
      );
    } else {
      this._restartPolicy =
  ...
  private _getDiagnostics(
    ...
    if (logAnalyticsWorkspace || logAnalyticsWorkspaceKey) {
      if (!logAnalyticsWorkspaceKey || !logAnalyticsWorkspace) {
        throw Error(
          "The Log Analytics Workspace Id or Workspace Key are not provided. Please fill in the appropriate parameters."
        );
      }
      if (
        logType &&
        !["ContainerInsights", "ContainerInstanceLogs"].includes(logType)
      ) {
        throw Error(
          "Log Type Can be Only of Type `ContainerInsights` or `ContainerInstanceLogs`"
        );
      }
      let logAnalytics: ContainerInstanceManagementModels.LogAnalytics = {
  ...
  private _getEnvironmentVariables(
    ...
    if (environmentVariables) {
      ...
      let keyValuePairs =
        environmentVariables.match(/(?:[^\s"]+|"[^"]*")+/g) || [];
    ...
    if (secureEnvironmentVariables) {
      // split on whitespace, but ignore the ones that are enclosed in quotes
      let keyValuePairs =
        secureEnvironmentVariables.match(/(?:[^\s"]+|"[^"]*")+/g) || [];
      keyValuePairs.forEach((pair: string) => {
        // value is either wrapped in quotes or not
        let pairList = pair.split(/=(?:"(.+)"|(.+))/);
        let obj: ContainerInstanceManagementModels.EnvironmentVariable = {
  ...
  private _getVolumes(
    gitRepoVolumeUrl: string,
    afsShareName: string,
    afsAccountName: string
  ) {
    ...
    if (gitRepoVolumeUrl) {
      ...
      let vol: ContainerInstanceManagementModels.GitRepoVolume = {
        repository: gitRepoVolumeUrl,
      };
      if (!gitRepoMountPath) {
      ...
      let volMount: ContainerInstanceManagementModels.VolumeMount = {
        name: "git-repo-vol",
        mountPath: gitRepoMountPath,
      };
    ...
    if (afsShareName && afsAccountName) {
      ...
      if (!afsMountPath) {
        throw Error(
          "The Mount Path for Azure File Share Volume is not specified"
        );
      }
      let vol: ContainerInstanceManagementModels.AzureFileVolume = {
        shareName: afsShareName,
        storageAccountName: afsAccountName,
      };
      if (afsAccountKey) {
      ...
      let volMount: ContainerInstanceManagementModels.VolumeMount = {
        ...
        mountPath: afsMountPath,
      };
      if (afsReadOnly) {
        if (!["true", "false"].includes(afsReadOnly)) {
          throw Error(
            "The Read-Only Flag can only be `true` or `false` for the Azure File Share Volume"
          );
        }
      ...
      throw Error(
        "The Name of the Azure File Share is required to mount it as a volume"
      );
    } else if (!afsAccountName && afsShareName) {
      throw Error(
        "The Storage Account Name for the Azure File Share is required to mount it as a volume"
      );
    } else {
    }
  }
